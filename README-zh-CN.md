...
一个高性能的 CLI、助手 skill 和 MCP（模型上下文协议）服务器，用于代码库索引、语义搜索和提示词增强，使用 Rust 编写。

## 概述

ace-tool-rs 是一个 Rust 实现的代码库上下文引擎，使 AI 助手和本地工作流能够使用自然语言查询来搜索和理解代码库。它提供：

- **一等 CLI 工作流** - 索引、搜索、提示词增强、运行 MCP 和安装本地助手 skill
- **实时代码库索引** - 自动索引项目文件并保持索引更新
- **语义搜索** - 使用自然语言描述查找相关代码
- **提示词增强** - 结合代码库上下文增强用户提示词，使请求更清晰、更可操作
- **助手 skill 分发** - 内置适用于 Codex、Claude 和 Pi 的 skill 指令
...
- **CLI 子命令** - `mcp`、`index`、`search`、`enhance` 和 `install-skill`
...
```bash
# 运行 MCP 服务器。MCP 模式下 enhance_prompt 默认不打开浏览器 UI。
...
| 参数 | 描述 |
|------|------|
| `mcp` | 通过 stdio 运行 MCP 服务器 |
| `index --project-root <path>` | 索引项目并退出 |
| `search --project-root <path> --query <text>` | 搜索项目上下文并输出结果 |
| `enhance --prompt <text> [--conversation-history <text>]` | 增强提示词并输出结果 |
| `install-skill --agents codex,claude,pi [--force]` | 将 `skills/ace-tool-rs` 安装到本机助手 skill 目录 |

### 运行说明

- `index` 要求项目目录已存在。对有效项目执行索引时，会在项目下写入 `.ace-tool/` 本地运行状态，并可能更新项目根目录 `.gitignore`，让这些状态文件被忽略。
- `search` 和 `search_context` 的返回结果适合作为定位线索。修改或引用精确实现前，请以本地文件内容为准。
- MCP 服务模式在未提供 prompt UI 参数时，默认让 `enhance_prompt` 直接返回结果、不打开浏览器。单次 `enhance` 仍默认保留浏览器审阅流程，除非传入 `--no-webbrowser-enhance-prompt`。
...
仓库内置轻量级 agent skill：`skills/ace-tool-rs`。构建或安装 CLI 后，可以安装到本机：
...
如果目标 skill 已存在，使用 `--force` 只替换 `ace-tool-rs` 这个目录：

```bash
ace-tool-rs install-skill --agents codex,claude,pi --force
```

skill 不保存 API 凭据。请通过运行时 CLI 参数、环境变量或你自己的密钥管理方式提供凭据。
...
如果你更喜欢 MCP 调用也走浏览器手动审阅，请显式传入 prompt UI 参数，例如 `--webui-addr 127.0.0.1:8754`，并在期待 MCP 调用结束之前先完成 Web UI 中的确认步骤。
...
搜索结果应视为定位线索。修改或引用精确代码、行号和行为前，请回到本地仓库文件确认。
...
- MCP 服务模式下，如果没有提供 prompt UI 参数，`enhance_prompt` 会直接返回 API 结果。
- 单次 CLI `enhance` / `--enhance-prompt` 模式仍默认使用浏览器审阅流程。
- 如果为 MCP 调用启用了浏览器审阅，等待确认期间 MCP 客户端可能看起来像是“卡在 send 之后不动了”。

**如果你希望完全在终端内 / 不弹浏览器：**

- 启动 ace-tool-rs 时加上 `--no-webbrowser-enhance-prompt`。
- 在这个模式下，`enhance_prompt` 会直接返回 API 结果，不会打开浏览器。
...
# 使用第三方端点的单次 enhance 模式下，--base-url 和 --token 是可选的
export PROMPT_ENHANCER_ENDPOINT=claude
export PROMPT_ENHANCER_BASE_URL=https://api.anthropic.com
export PROMPT_ENHANCER_TOKEN=your-anthropic-api-key
...
- 在 `enhance` / `--enhance-prompt` 单次模式下，如果启用了这个开关，也必须额外提供 `--base-url` 和 `--token`
...
- 需要网络访问索引 API
- 索引会写入 `.ace-tool/`，并可能更新项目根目录 `.gitignore`
- 最大文件大小：每个文件 500KB
- 最大批次大小：每次上传批次 5MB
